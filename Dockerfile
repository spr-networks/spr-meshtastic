...
WORKDIR /code
COPY code/ /code/
RUN --mount=type=tmpfs,target=/root/go/ go build -trimpath -ldflags "-s -w" -o /meshtastic_plugin /code/
...
ENV DEBIAN_FRONTEND=noninteractive
ARG UBUNTU_SNAPSHOT=20260601T000000Z
ARG MESHTASTIC_VERSION=2.7.10
# apt only from the snapshot archive so the runtime layer is reproducible too
RUN set -eux; \
    printf 'Types: deb\nURIs: https://snapshot.ubuntu.com/ubuntu/%s\nSuites: noble noble-updates noble-security\nComponents: main restricted universe multiverse\nSigned-By: /usr/share/keyrings/ubuntu-archive-keyring.gpg\n' "${UBUNTU_SNAPSHOT}" > /etc/apt/sources.list.d/ubuntu.sources; \
    printf 'APT::Install-Recommends "false";\nAcquire::Check-Valid-Until "false";\n' > /etc/apt/apt.conf.d/99reproducible
...
# Meshtastic CLI: every package (incl. transitive deps) pinned by version and
# sha256 in requirements.txt; pip refuses anything unpinned (--require-hashes).
# --no-build-isolation makes the one sdist (pyqrcode) build against the
# snapshot-pinned apt setuptools/wheel instead of pulling unpinned build deps.
COPY requirements.txt /requirements.txt
RUN set -eux; \
    grep -q "^meshtastic==${MESHTASTIC_VERSION} " /requirements.txt; \
    python3 -m venv --system-site-packages /opt/meshtastic; \
    /opt/meshtastic/bin/pip install --no-cache-dir --require-hashes --no-build-isolation -r /requirements.txt; \
    ln -s /opt/meshtastic/bin/meshtastic /usr/local/bin/meshtastic; \
    /opt/meshtastic/bin/meshtastic --version
COPY scripts /scripts/
COPY --from=builder /meshtastic_plugin /
