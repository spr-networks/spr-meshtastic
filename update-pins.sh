#!/usr/bin/env bash
# update-pins.sh — re-resolve every pin, rewrite reproducible.env + requirements.txt,
# and sync the matching `ARG <KEY>=` defaults / `# syntax=` line in the Dockerfile(s).
# Read-only lookups (registry manifest inspect, go.dev, pypi.org). Review with `git diff`.
# Tip: `docker login` first to avoid Docker Hub's unauthenticated pull-rate-limit (429).
# Needs: docker buildx, curl, python3, uv (https://docs.astral.sh/uv/ — for the
# hash-checked Python requirements).
set -euo pipefail
cd "$(dirname "$0")"
...
GO_MINOR=1.25
# Python version shipped by the runtime image (ubuntu noble)
PYTHON_TARGET=3.12
...
# ---- meshtastic Python CLI: latest release + hash-checked requirements.txt ----
# MESHTASTIC_VERSION=x.y.z ./update-pins.sh pins a specific release instead.
MESHTASTIC_VERSION="${MESHTASTIC_VERSION:-$(curl -fsSL https://pypi.org/pypi/meshtastic/json \
  | python3 -c 'import json,sys; print(json.load(sys.stdin)["info"]["version"])')}"
echo "Resolving Python requirements for meshtastic==${MESHTASTIC_VERSION} (py${PYTHON_TARGET}, linux amd64+arm64)..." >&2
command -v uv >/dev/null || { echo "uv is required to regenerate requirements.txt" >&2; exit 1; }
tmpd=$(mktemp -d)
trap 'rm -rf "$tmpd"' EXIT
printf 'meshtastic[cli]==%s\n' "$MESHTASTIC_VERSION" > "$tmpd/requirements.in"
uv pip compile "$tmpd/requirements.in" --python-version "$PYTHON_TARGET" \
  --python-platform x86_64-unknown-linux-gnu --generate-hashes --no-header \
  -o "$tmpd/req-amd64.txt" >/dev/null
uv pip compile "$tmpd/requirements.in" --python-version "$PYTHON_TARGET" \
  --python-platform aarch64-unknown-linux-gnu --generate-hashes --no-header \
  -o "$tmpd/req-arm64.txt" >/dev/null
diff -u "$tmpd/req-amd64.txt" "$tmpd/req-arm64.txt" >&2 || {
  echo "amd64/arm64 resolutions differ — pin per-arch requirements manually" >&2; exit 1; }
{
  echo "# Fully pinned, hash-checked requirements for the Meshtastic Python CLI."
  echo "# Install with: pip install --require-hashes -r requirements.txt"
  echo "# Regenerate with ./update-pins.sh (uses \`uv pip compile\` cross-resolved for"
  echo "# linux/amd64 + linux/arm64, Python ${PYTHON_TARGET} — see the recipe there)."
  cat "$tmpd/req-amd64.txt"
} > requirements.txt
...
# meshtastic Python CLI (pip): version pinned here for reference/CI; the full
# dependency set with sha256 hashes lives in requirements.txt (--require-hashes).
